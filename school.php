...
require_once('config/database_connector.php');
...
$level='00'; //new user record to be inserted
$user_name = 'Guest';
...
if($level != null || $level != ''){
  ...
  if($userResponse == '99'){  //if user selected option 99
    ...
    VALUES('".$phoneNumber."','".$sessionId."','99','".$response."','Quit' )";
    $db->query($sql1);
    ...
    //create session record
      ...
      $response .= " 1. Timetable\n";
      $response .= " 2. Fee Structure\n";
      $response .= " 3. Fee Statement\n";
      $response .= " 4. Exam Results\n";
      $response .= " 5. Memo\n";
      $response .= " 6. Library Information\n";
      $response .= " 00. Home \n";
      $response .= " 99. Quit ";
  ...
  switch($level){
    case '0':
      if($userResponse != null || $userResponse != ''){
        ...
        }else if($userResponse == '3'){ //if user selects my account option
        ...
        }else if($userResponse == '4'){ //if user selects my account option
        ...
        }else if($userResponse == '5'){ //if user selects my account option
        ...
        }else if($userResponse == '6'){ //if user selects my account option
    ...
    //case 00 - Return to main homepage
    case '00':
    //create session record
      $sql1 = "INSERT INTO `response`(`PhoneNumber`,`sessionID`,`level`,`levelText`)
      VALUES('".$phoneNumber."','".$sessionId."','0','New Session Created' )";
